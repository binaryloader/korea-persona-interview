...
엔비디아가 2025년 공개한 `nvidia/Nemotron-Personas-Korea` 데이터셋은 약 100만 레코드, 약 700만 합성 페르소나를 CC BY 4.0 라이선스로 제공한다. 인구 통계 분포에 가깝게 합성된 한국인 페르소나에 로컬 LLM(Apple Silicon + MLX)을 결합하면, 실제 인터뷰 이전 단계의 빠른 가설 검증과 페르소나별 반응 시뮬레이션이 가능하다. 외부 API 비용이 없고 데이터가 로컬에서만 흐르므로 사업 아이템 정보가 외부로 노출될 위험도 낮다.

본 도구는 진짜 인터뷰를 대체하지 않는다. 진짜 인터뷰 직전 단계의 가설 정리, 질문지 점검, 초기 페르소나 가설 수립을 빠르게 반복하는 용도로 한정한다. 결과의 한계와 합성 페르소나라는 사실을 사용자 인터페이스와 출력물 모두에서 명시한다.
...
- 추론은 전부 로컬에서 수행한다. 외부 LLM API 호출, 외부 텔레메트리, 외부 분석 서비스 의존을 금지한다
...
- Given MLX 서버가 `http://localhost:8080`에서 정상 동작 중일 때, When 사용자가 `python main.py healthcheck`를 실행하면, Then 종료 코드 0과 함께 사용 가능한 모델 ID가 출력된다
- Given MLX 서버가 꺼져 있거나 포트가 다를 때, When 사용자가 `python main.py healthcheck`를 실행하면, Then 종료 코드 1과 함께 "MLX 서버가 응답하지 않습니다. 별도 터미널에서 `mlx_lm.server --model {model} --port 8080`을 실행해 주세요"라는 친절한 한국어 안내가 출력된다
...
- 기본 묶음(항상 주입): 데이터셋의 인구 통계 필드(이름, 성별, 연령, 혼인 상태, 교육, 직업, 거주 지역) + summary 페르소나(전체 요약 자유 서술)
...
- 페르소나 깨짐 감지: 응답 텍스트의 영어 단어 비율이 30%를 초과하거나, 페르소나의 연령/성별/지역과 명백히 모순되는 자기소개(예: 70대 페르소나가 "저는 20대 학생인데"라고 시작)가 발견되면 `flags.persona_drift: true`와 `status: "drift"`로 기록한다. 모순 감지는 휴리스틱(연령대 키워드, 성별 키워드, 지역명 정면 부정 표현)으로 한정한다(LLM 기반 감지는 비용 문제로 v1 제외)
...
- 호출 실패: 타임아웃 120s, HTTP 5xx, 연결 실패 시 지수 백오프(1s, 2s, 4s)로 최대 3회 retry한다. 최종 실패 시 `status: "failed"`와 `error.message`를 기록하고 다음 페르소나로 진행한다
- 필터 결과 0건: 인터뷰 시작 전에 종료 코드 2와 함께 안내한다(§4.2)
- MLX 서버 다운: 인터뷰 시작 직전 헬스체크에서 감지하면 인터뷰를 시작하지 않는다(§4.1)
...
| --- | --- | --- | --- |
| `healthcheck` | MLX 서버 응답과 모델 가용성 확인 | `--base-url`(기본 `http://localhost:8080/v1`) | 0 정상, 1 서버 다운 |
...
- 게이트 1(MLX 서버 기동 확인): `llm_client.py`를 작성하고 sanity 호출(`/v1/models` GET, "안녕" 메시지 POST)을 한 시점에 사용자에게 "MLX 서버 띄워주세요. 모델 ID와 포트 알려주세요" 안내. 응답이 정상 200으로 확인되기 전까지 다음 단계 진행을 막는다
...
- 100명 인터뷰 1회를 30분 이내에 완료한다(질문 5개, 평균 응답 4초, 동시성 2 가정)
- 데이터셋 첫 로드는 5분 이내에 완료한다(`~/.cache/huggingface` 캐시 활용). 두 번째 실행부터는 30초 이내에 시작한다
- 동시성 기본값은 2로 둔다. `asyncio.Semaphore(2)` 기준이다. 사용자가 `--concurrency` 옵션으로 1-3 범위에서 조정할 수 있다. 4 이상은 OOM 위험으로 차단한다
...
- API 키 입력 코드를 일체 포함하지 않는다(로컬 전용)
- `Authorization` 헤더는 생략하거나 더미값을 사용한다
- 외부 텔레메트리, 외부 분석 서비스, 외부 LLM API 호출을 일체 포함하지 않는다
- 인터뷰 결과 JSON에 사용자 개인 식별 정보를 기록하지 않는다(합성 페르소나만 기록)
- 사업 아이템 설명(`--product`)은 로컬 파일에만 저장된다. 외부로 송신되지 않는다
...
- Python 3.10 이상에서 동작한다
- Apple Silicon에서 MLX 서버를 사용한다(외부 GPU/Linux는 v1 범위 밖)
- 의존성은 `httpx`, `datasets`, `pyyaml`, `tqdm`, `click`로 한정한다. `anthropic`, `openai` SDK는 사용하지 않는다(`dependency.md` §1 leftpad 안티패턴 회피와 직접 통제 목적)
- 모든 의존성 버전은 `requirements.txt`에 안정 버전으로 고정한다(`dependency.md` §2)

### 6.6. 관측 가능성

- 표준 출력에 진행률(tqdm)과 단계별 INFO 로그를 출력한다
- 로그는 구조화된 형태(JSON Lines)로 `outputs/logs/run_{timestamp}.jsonl`에도 동시 기록한다
- 민감 정보(사용자가 `--product`에 적은 사업 아이템 본문)는 로그 본문에 그대로 기록하지 않고 해시 또는 첫 30자 + 길이 형태로 마스킹한다
...
- 위험: 사용자가 별도 터미널에서 MLX 서버를 띄워야 하고, 서버 상태를 도구가 직접 통제하지 못한다
- 완화: 매 명령 시작 직전 헬스체크 수행, 친절한 한국어 안내 메시지로 사용자가 즉시 조치 가능하도록 한다

### 10.5. 컴퓨터 메모리 한계

- 위험: 동시성을 높이거나 큰 모델을 사용하면 OOM이 발생한다
- 완화: 동시성 1-3 범위 강제, OOM 발생 시 친절한 안내(동시성 낮추기, 모델 변경 제안), README에 메모리 권장 사양 명시
...
- 위험: 핸드오프에서 권장한 모델(`unsloth/Qwen3.6-35B-A3B-UD-MLX-4bit`)이 사용자 환경에 없거나 다른 모델로 교체될 수 있다
- 완화: 모델 ID는 `config.yaml`에서만 변경하면 동작하도록 설계, 게이트 1에서 사용자에게 사용 모델을 확인 후 진행
