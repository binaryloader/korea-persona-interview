...
- 동시성 기본값은 4로 둔다. `asyncio.Semaphore(4)` 기준이다. 사용자가 `--concurrency` 옵션으로 1-10 범위에서 조정할 수 있다. 11 이상은 차단한다(OpenAI rate limit 부하와 비용 폭증 방지). v1.0 시절 1-3 범위는 로컬 MLX 메모리 가드였고, OpenAI 백엔드에서는 메모리 가드가 무관해 1-10으로 상향했다
...
- 완화: gpt-4o-mini 기본값 유지, 동시성 1-10 범위 강제, 멀티턴 토큰 윈도우 8000 truncation 정책 유지(TDD §7), 토글 옵션 기본값을 `summary` 1종으로 제한, README의 "Limitations and Disclaimer"에 비용 추정과 권장 운영 가이드를 명시한다. gpt-4o 같은 상위 모델은 사용자가 명시 선택할 때만 사용한다
