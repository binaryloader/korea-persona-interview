...
- 사업 아이템 본문(`--product`)과 페르소나 정보(인구 통계, 자유 서술)는 사용자가 설정한 LLM 백엔드로 송신된다. 백엔드는 `provider`/`base_url`/`mcp.mode` 조합에 따라 OpenAI Chat Completions API, Anthropic Messages API, OpenAI 호환 로컬 서버(mlx_lm.server, vLLM, llama.cpp 등), 또는 MCP 호스트 에이전트의 LLM 중 하나가 된다. 사용자가 도구 실행 전에 본 사실을 인지해야 한다. 민감한 사업 아이템(미공개 IP, 개인정보 포함)은 입력하지 않거나 추상화하여 입력하기를 권고한다. 본 사실은 README와 도구 첫 실행 메시지에서 명시한다
...
- 위험: `--product`에 적은 사업 아이템 본문과 페르소나 정보가 사용자가 설정한 LLM 백엔드(OpenAI / Anthropic / 로컬 LLM / MCP 호스트 에이전트)로 송신된다. 사용자가 본 사실을 인지하지 못하면 미공개 IP, 영업 비밀, 개인정보가 provider 외부로 노출될 수 있다. 로컬 LLM 또는 자체 호스팅 MCP 에이전트를 쓰면 외부 노출 면이 줄어들지만 백엔드 호출 자체는 동일하게 발생한다
- 완화: README와 도구 첫 실행 메시지에 외부 송신 사실을 명시, 민감 정보 입력 자제 권고. provider별 데이터 이용 정책 차이는 사용자가 직접 확인하도록 안내한다. v1.1.0에서 OpenAI 호환 로컬 서버(mlx_lm.server / vLLM / llama.cpp) 경로를 정식 지원해 외부 송신을 회피하고 싶은 사용자가 선택할 수 있게 했다
