...
- 100명 인터뷰 1회를 30분 이내에 완료한다(질문 5개, 동시성 2 가정). gpt-4o-mini 기준 한 턴 응답이 약 1-3초로 추정되며 30분 SLO에 여유가 크다. 로컬 MLX(평균 4-10초/턴) 대비 처리 시간은 단축될 가능성이 높다
- 데이터셋 첫 로드는 5분 이내에 완료한다(`~/.cache/huggingface` 캐시 활용). 두 번째 실행부터는 30초 이내에 시작한다
- 동시성 기본값은 2로 둔다. `asyncio.Semaphore(2)` 기준이다. 사용자가 `--concurrency` 옵션으로 1-3 범위에서 조정할 수 있다. 4 이상은 차단한다(OpenAI rate limit 부하와 비용 폭증 방지)
...
- 처리 성능: 100명 인터뷰 1회를 30분 이내에 완료한다(질문 5개, 동시성 2 기준)
...
- 비용: gpt-4o-mini 기준 100명 인터뷰 1회의 OpenAI API 비용이 약 $0.50 - $2.00 범위 안에 들어온다(질문 5개 + 구조화 요약 + 정성 인사이트 1회 호출 가정). 5달러를 초과하면 프롬프트와 토글 옵션을 점검한다
...
### 10.5. OpenAI API 비용 증가

- 위험: 멀티턴 토큰 누적, 토글 옵션 다수 사용, 동시성 상향, 큰 모델(`gpt-4o`) 선택 시 비용이 빠르게 늘어날 수 있다. 100명 배치 1회의 비용이 의도와 다르게 5달러 이상으로 튀면 도구의 활용 가치를 희석시킨다
- 완화: gpt-4o-mini 기본값 유지, 동시성 1-3 범위 강제, 멀티턴 토큰 윈도우 8000 truncation 정책 유지(TDD §7), 토글 옵션 기본값을 `summary` 1종으로 제한, README의 "Limitations and Disclaimer"에 비용 추정과 권장 운영 가이드를 명시한다. gpt-4o 같은 상위 모델은 사용자가 명시 선택할 때만 사용한다
