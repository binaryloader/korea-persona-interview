...
| GATE-1 | MLX 서버 기동 휴먼 검증 | 사용자 검증 단계 | 0.5h | T3 | 사용자와 메인 세션 | PRD §5.10 게이트 1. 사용자가 별도 터미널에서 `mlx_lm.server`를 띄우고 `python main.py healthcheck` 또는 임시 sanity 스크립트로 `/v1/models` 200 응답과 짧은 hello dry-run 1회를 확인한다. 통과 전까지 T5 진입을 막는다 |
...
| T10 | README | `README.md` | 3h | T8 | tech-writer 또는 ml-engineer | git.md §4.2. 영어로 작성한다. 구조는 Title, Description, Features, Requirements, Installation, Usage, Dataset/Model Credits, Synthetic Data Disclaimer, License 순이다. Requirements는 Apple Silicon 16GB+, Python 3.10+, mlx-lm 별도 설치를 명시한다. Usage는 4개 서브커맨드 예시를 포함한다. Credits는 엔비디아 `nvidia/Nemotron-Personas-Korea`와 CC BY 4.0을 명시한다. Disclaimer는 실제 인터뷰 대체 아님을 명시한다. License는 MIT다. 첫 로드 5-10분 안내, 메모리 권장 사양, mlx_lm.server 기동 명령 예시를 포함한다 |
...
flowchart TD
    ...
    T2 --> T3[T3 LLM 클라이언트]
    T2 --> T4[T4 페르소나 로더]
    T3 --> GATE1{{GATE-1<br/>MLX 서버<br/>휴먼 검증}}
...
- M1은 T1, T2, T3, GATE-1을 포함한다. 인프라 준비 완료, MLX 서버 응답 확인, healthcheck 명령 일부 동작이 가능하다
...
- T8의 표 형식 출력은 한글 폭 2 가정 단순 정렬로 v1을 시작한다. 정렬 깨짐이 발생하면 v1.1에서 wcwidth 의존을 도입한다. UI §5.4를 참고한다
