...
- ``OpenAIBackend``, ``AnthropicBackend``: ``LLMBackend`` 프로토콜을 만족하는 LLM 백엔드 구현(CLI와 MCP server 모드에서 사용)
- ``load_config``, ``AppConfig``: 레이어드 설정 로더

CLI 진입점은 ``main.py``를 통해 노출된다(콘솔 스크립트 ``kpi``). MCP stdio 서버는 ``src.mcp_server``에 있다(콘솔 스크립트 ``kpi-mcp-server``).
"""
