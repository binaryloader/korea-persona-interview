...
성공/에러 응답 모두 ``ok: true|false`` 필드와 ``backend`` 라벨이 박힌
공통 dict 형태를 따른다. 호출자가 단일 키로 분기 가능하도록 하기 위함이다
(README Integration 섹션 참조).
"""
...
def persona_to_payload(persona: PersonaMeta) -> dict:
    ...
    list_personas와 build_persona_prompt 도구가 동일한 모양으로 페르소나를
    노출하기 위해 한 곳에서 직렬화한다.
    """
